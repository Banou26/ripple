...
export const createThumbnailGenerator = async ({ publicPath, workerUrl, length, read, onThumbnails }: ThumbnailGeneratorOptions) => {
  ...
  type Slot = { timestamp: number, endTime: number, startByte: number, endByte: number, done: boolean }
  const slots: Slot[] = []
  for (const [i, index] of metadata.indexes.entries()) {
    ...
    slots.push({
      ...
      endTime: duration,
      startByte: index.pos,
      endByte: metadata.indexes[i + 1]?.pos ?? length,
      done: false,
    })
  }
...
export const useSeekThumbnails = ({ enabled, publicPath, workerUrl, length, read, ranges }: {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const genPromise = createThumbnailGenerator({
      publicPath,
      workerUrl,
      length,
    ...
    genPromise.then((gen) => {
      if (cancelled) { gen.destroy(); return }
      genRef.current = gen
      gen.update(rangesRef.current)
    }, () => {})
    return () => {
      cancelled = true
      genRef.current = null
      genPromise.then((gen) => gen.destroy(), () => {})
      setThumbnails([])
    }
